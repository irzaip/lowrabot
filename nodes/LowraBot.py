"""
A Python implementation of the Wowwee LowraBot web-based API.

The LowraBot mobile webcam is controlled through a web-based API.  The LowraBot class
wraps http calls to a LowraBot and returns the appropriate responses.  It also
provides some additional support methods for parsing the responses.

Classes:
  - LowraBot: Access to an instance of a LowraBot mobile webcam

Exceptions:
  - LowraBotError: base class for LowraBot-related exceptions

Handlers:
  - NullHandler: do-nothing handler for logging

Module Attributes:
  - lowrabots: a map of LowraBot names to LowraBot objects
  - rlog: logging.Logger object for logging

Module Functions:
  - getLowraBot: return the lowrabot object with the given name

Module Constants:
  - __version__: The version of the PyLowraBot interface module as a string
  - API_VERSION: The version of the LowraBot API as a string
  - API_DATE: Release date of the LowraBot API
  - INFO: PyLowraBot version and API info as a string
  ...
  - response_codes: map of response codes to [name, docstring]
    ...
    These are returned by many LowraBot commands.
...
PyLowraBot is developed at the University at Buffalo and distributed under the UB
...
INFO = 'PyLowraBot v%s ; LowraBot API v%s released %s' % (__version__, API_VERSION,
                                                    API_DATE)
...
USER_AGENT = 'PyLowraBot/%s' % __version__
...
lowrabots = dict()
"""Map of LowraBot names to interface objects"""
rlog = logging.getLogger('lowrabot')

####################
# MODULE FUNCTIONS #
####################

def getLowraBot(name):
    """Return the LowraBot object named by name."""
    return lowrabots[name]
...
class LowraBotError(Exception):
    """Base class for errors in the LowraBot package."""

class ConnectError(LowraBotError):
    """
    Exception raised for error connecting to the LowraBot.

    Attributes:
      - lowrabot:   LowraBot object
      - message: explanation of the error

    """

    def __init__(self, lowrabot):
        self.lowrabot = lowrabot
        self.message = ('Error connecting to %s (host: %s)' %
                        (self.lowrabot.name, self.lowrabot.host))

class ResponseError(LowraBotError):
    """
    ...
    def __init__(self, lowrabot, code):
        self.lowrabot = lowrabot
        self.code = code
...
class ParamError(LowraBotError):
    """
    ...
    def __init__(self, lowrabot, param, value, message=None):
        self.lowrabot = lowrabot
        self.param = param
        self.message = ('Parameter error from %s: Attempting to set param %s '
                        'to %s (Details: %s)') % (self.lowrabot.name,
                                                  self.param,
                                                  self.message)
...
class OutOfRangeError(ParamError):
    ...
    def __init__(self, lowrabot, param, range_, value):
        super(OutOfRangeError, self).__init__(lowrabot, param, value,
...
class LowraBot:
    ...
    """
    An instance of the LowraBot class provides an interface to one LowraBot.

    The LowraBot API consists of function calls made over HTTP.  This class wraps
    the HTTP requests with Python method calls.  The LowraBot API is mirrored as
    ...
    rather than parameters to the ManualDrive method in the LowraBot API.

    You can set the hostname of the LowraBot to connect to using the host
    property.  You can also set the IP address or host of the LowraBot webcam
    itself using the LowraBot API using SetHTTP.  After using SetHTTP, you are
    required to then set the host property to the same address in order to
    continue controlling the same LowraBot object.  (Note: This was an arbitrary
    design decision in making the LowraBot class.)  TODO: example
    ...
    LowraBot's webserver, so set parameters carefully.

    Properties:
      - host:     hostname or IP address of the LowraBot
      - name:     name of this LowraBot (read-only)
      - port:     HTTP port number (default 80)
      - protocol: Protocol to use (read-only, default http)
      - speed:    Default LowraBot speed (1 fastest, 10 slowest, default 1)
    ...
    Commands:
      ...
      - get_MCU_report
      - get_path_list
      - get_report:           return a status report on the LowraBot
      - get_status
    ...
    optional speed parameter that defaults to the default speed of this LowraBot
    object.
    ...
    Documentation taken from the API Specification for LowraBot, version 1.2,
    ...
    speed = property(get_speed, set_speed,
                     doc="""
                     LowraBot's default movement speed.
    ...
    name = property(get_name, doc="""Name of the LowraBot the object represents""")
    ...
    host = property(get_host, set_host,
                    doc="""Hostname or IP address of the LowraBot""")
    
    def __init__(self, name, host, username=None, password=None, port=80):
        """
        Initialize a new LowraBot interface.

        Parameters:
          - name:     name of this LowraBot mobile webcam
    ...
    def forward(self, speed=None):
        """Move LowraBot forward."""
        return self.manual_drive(1, speed)

    def backward(self, speed=None):
        """Move LowraBot backward."""
        return self.manual_drive(2, speed)

    def left(self, speed=None):
        """Move LowraBot straight left."""
        return self.manual_drive(3, speed)

    def right(self, speed=None):
        """Move LowraBot straight right."""
        return self.manual_drive(4, speed)

    def rotate_left(self, speed=None, angle=None):
        """
        Rotate LowraBot left by speed.

        The optional angle parameter turns the LowraBot a certain distance.
    ...
    def rotate_right(self, speed=None, angle=None):
        """
        Rotate LowraBot right by speed.
    ...
    def enable_IR(self):
        """Enable IR sensor."""
        return self.manual_drive(19,0,0,'IR',1)

    def disable_IR(self):
        """Disable IR sensor."""
        return self.manual_drive(19,0,0,'IR',0)

    def enable_Light(self):
        """Enable Light."""
        return self.manual_drive(19,0,0,'LIGHT',1)

    def disable_Light(self):
        """disable light."""
        return self.manual_drive(19,0,0,'LIGHT',0)

    def get_report(self):
        """
        Get LowraBot's current status.

        Generate a report from libNS module that provides LowraBot's current
        status.  Return a dictionary (keys are strings).

        Key                Description
        -----------------------------------------------------------------------
        responses          error checking (0: no error)
        x, y, theta        average location of LowraBot in relation to the
    ...
    def start_recording(self):
        ...
        LowraBot will resist going outside NorthStar (navigation system) coverage
        area while recording path.

        LowraBot will stop recording if coverage is lost.

        LowraBot will stop recording if user connection is lost.
    ...
    def get_path_list(self):
        """
        Return a list of paths stored in the LowraBot.
    ...
    def get_MCU_report(self):
        ...
        13     1B     0x7F: battery full (0x7f or higher for new battery)
                      0x??: orange light in LowraBot head (to be defined)
    ...
    def get_data(self):
        ...
        LowraBot API documentation on this command is not very good.  Does nothing
        at the moment.
    ...
    def get_image(self, imgID = None):
        """
        Acquire an image from the LowraBot webcam.
    ...
    def change_resolution(self, ResType=2, RedirectURL=None):
        ...
        Requires administrative privileges on the LowraBot.
    ...
    def change_speaker_volume(self, SpeakerVolume=15, RedirectURL=None):
        """
        Change the speaker volume of the LowraBot.
    ...
    def change_mic_volume(self, MicVolume=15, RedirectURL=None):
        """
        Change the microphone volume of the LowraBot.
    ...
    def manual_drive(self, command, speed=None, angle=None, device=None, state=None):
        """
        Send a movement command to the LowraBot.
        ...
        # turn on or off LED or IR
        elif command == 19:
            page = ('rev.cgi?Cmd=nav&action=19&%s=%d' %
                    (device, state))
    ...
    def _get_request_response(self, page):
        """
        Send a command to the LowraBot and return its response.

        In general, this command should not be called directly.

        Parameters:
          - page: the LowraBot API command to request
        ...
        f = urllib2.urlopen(url=req,timeout=.4)
        data = f.read()
        return data;

    def _parse_response(self, response):
        """
        Parse the response of some LowraBot CGI commands.
...
class LowraBotController(threading.Thread):

    """
    Controls the LowraBot robot.

    A higher-level wrapper for the API.

    Attributes:
      - lowrabot: the LowraBot being controlled (read-only)
      - wait: the amount of time to sleep before checking the LowraBot event queue

    """

    def getLowraBot(self): return self._lowrabot
    lowrabot = property(getLowraBot, doc="""LowraBot being controlled (read-only)""")

    def __init__(self, lowrabot):
        """
        Initialize a LowraBotController.

        Parameters:
          - lowrabot: lowrabot object to be controlled

        """
        threading.Thread.__init__(self)
        self._lowrabot = lowrabot
        self._running = True
        self._queue = []
        self.wait = 0.1
